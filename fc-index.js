...
function callBailian(appId, messages) {
  return new Promise(function(resolve, reject) {
    ...
    var req = https.request(options, function(res) {
      ...
      res.on('end', function() {
        try {
          if (res.statusCode !== 200) reject(new Error('Bailian HTTP ' + res.statusCode));
...
var LOADER = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>PM面试教练</title></head><body style="background:#07070f;display:flex;align-items:center;justify-content:center;height:100vh;margin:0"><div style="color:#a78bfa;font-family:sans-serif;font-size:18px">加载中...</div><script>fetch(location.href.split("?")[0]+"?app=1").then(function(r){return r.text()}).then(function(h){document.open();document.write(h);document.close()}).catch(function(){document.body.innerHTML='<div style="color:#fb7185;font-family:sans-serif;text-align:center"><p>加载失败</p><p style="font-size:14px;color:#9ca3af">请刷新重试</p></div>'})</script></body></html>`;
...
async function submitResume(){
  ...
  try{
    var r=await fetch(API_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:"resume",messages:[{role:"user",content:"[简历] "+text}]})});
    var d=await r.json();
...
exports.handler = async function(event, context, callback) {
  try {
    ...
    if (method === 'POST') {
      ...
      callback(null, ok('application/json', {
    ...
    // GET: 小加载页 → 浏览器会显示(不下载) → JS自动拉取完整应用
    if (raw.queryString && raw.queryString.indexOf('app=1') !== -1) {
      callback(null, ok('text/html; charset=utf-8', APP));
    } else {
      callback(null, ok('text/html; charset=utf-8', LOADER));
    }
  } catch (e) {
